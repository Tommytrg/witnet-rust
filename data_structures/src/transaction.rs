...
impl StakeTransactionBody {
    ...
    pub fn new(inputs: Vec<Input>, output: StakeOutput, change: Option<ValueTransferOutput>) -> Self {
    ...
    pub fn weight(&self) -> u32 {
        let inputs_len = u32::try_from(self.inputs.len()).unwrap_or(u32::MAX);
        let inputs_weight = inputs_len.saturating_mul(INPUT_SIZE);
        let change_weight = if self.change.is_some() { OUTPUT_SIZE } else { 0 };

        inputs_weight.saturating_add(change_weight).saturating_add(STAKE_OUTPUT_SIZE)
    }
}

...
pub fn validate_block_transactions(
    ...
    for i in 1..block.txns.stake_txns.len() {
        let found = block.txns.stake_txns[i..].iter().find(|stake_tx| {
            ...
            stake_tx.body.output.authorization.public_key == stake_tx_aux.body.output.authorization.public_key
        });

        // TODO: refactor
        if found.is_some() {
            return Err(BlockError::RepeatedStakeOperator { pkh: found.unwrap().body.output.authorization.public_key.pkh().hash() }.into());
        }
    }
    
    // let mut operators = HashMap::new();
    // for transaction in &block.txns.stake_txns {
    //     operator =  &transaction.body.output.authorization.public_key
    //         operators
    //             .entry(signature.public_key.clone())
    //             .and_modify(|times| *times += 1)
    //             .or_insert(1);
    //     }
    // let mut repeated_operators = Vec::new();
    // for operator in operators {
    //     if (operator.1) != 1 {
    //         repeated_operators.push(operator.0);
    //     // return error here
    //     }
    // }
